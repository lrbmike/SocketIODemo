...
public class SSLSocketChannel2 implements ByteChannel, WrappedByteChannel {
	...
	private synchronized void processHandshake() throws IOException {
		...
		if( engineResult.getHandshakeStatus() == HandshakeStatus.NEED_UNWRAP ) {
		...
		if( tasks.isEmpty() || engineResult.getHandshakeStatus() == HandshakeStatus.NEED_WRAP ) {
	...
	private synchronized ByteBuffer unwrap() throws SSLException {
		...
		} while ( engineStatus == Status.OK && ( rem != inData.remaining() || engineResult.getHandshakeStatus() == HandshakeStatus.NEED_UNWRAP ) );
